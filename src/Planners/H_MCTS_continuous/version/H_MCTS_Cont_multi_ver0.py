...
class H_MCTS_Cont:
    ...
    def search(self, tree_save_path=None, traj_save_path=None):
        ...
        for i in range(self.searchLimit):
            ...
            if path is not None:
                success = True
                # print(self.root.children.values())
                self.cont_env.plot_grid_tree(self.root, save_path=Continuous_H-MCTS_extendable-debugging_wandb-real.ipynb_save_path)
                if traj_save_path:
                    sys.stdout.close()
    ...
    def delete_child(self, node_start: H_Node_Cont, subgoal_traj: list):
        for action in node_start.children.keys():
                if node_start.children[action].s == subgoal_traj[0]:
                    # print("Deleting", node_start.children[action].s)
                    # print("Deleting", subgoal_traj[0])
