...
from src.Planners.H_MCTS_continuous.Node_Cont_ignore_2 import H_Node_Cont
from src.Env.Grid.Cont_Grid import Continuous_Grid

# Recently Implementing version of Extendable Planning
# with added features
# 1. Refund when traversing
# 2. Do not remove goal_r when subgoal_r > 0
...
class H_MCTS_Cont:
    ...
    def search(self, tree_save_path=None, traj_save_path=None):
        ...
        for i in range(self.searchLimit):
            ...
            if path is not None:
                ...
                print("DRAW TREE")
                # self.visualize_tree(self.root, 0)
    ...
    def executeRound(self):
        ...
        if node.isTerminal:
            ...
            print("extend_cnt", self.extend_cnt)
    ...
    def backpropagate_wo_numVisits(self, node: H_Node_Cont, node_start: H_Node_Cont):
        reward = self.getRewardnRefund(node)
    ...
    def getReward2(self, node: H_Node_Cont):
        if node.s[0] != 0:
            subgoal_r = self.env.reward_subgoal(node)
            goal_r = self.env.reward_goal(node.s)
            return subgoal_r+goal_r
        else:
            subgoal_r = self.cont_env.reward_subgoal(node)
            goal_r = self.cont_env.reward_goal(node.s, node.parent.s)

            return subgoal_r+goal_r
    
    def getRewardnRefund(self, node: H_Node_Cont):
        if node.s[0] != 0:
            # Because subgoal_r is always > 0
            subgoal_r = self.env.reward_subgoal(node)
            assert(subgoal_r > 0)
            goal_r = self.env.reward_goal(node.s)
            if goal_r < 0:
    ...
    def TraverseNAddNewSubgoal(self, node_start: H_Node_Cont, node: H_Node_Cont, subgoal_traj: list, subgoal_following=False):
        ...
        for child in node.children.values():
            ...
            if high_level_child_s == subgoal_traj[0]:
                ...
                self.TraverseNAddNewSubgoal(node_start, child, subgoal_traj[1:], True)
            else:
