...
class H_MCTS_Cont:
    ...
    def search(self, tree_save_path=None, traj_save_path=None):
        ...
        for i in range(self.searchLimit):
            ...
            if (path is not None and self.stepbystep) or (self.stepbystep and i%100==1):
    ...
    def executeRound(self):
        ...
        if node.isTerminal:
            ...
            if self.stepbystep and subgoal_traj[0][0]!=2:
                input("Press Enter to continue...")
                self.visualize_tree(self.root)
    ...
    def draw_tree(self, node: H_Node_Cont, depth=0):
        ...
        if not node.isRoot:
            node_UCT = (
                            node.totalReward / (node.numVisits+1)
                            + self.explorationConstant_h
                            * math.sqrt(math.log(node.parent.numVisits + 1) / (node.numVisits+1))
                        )
            print(indent + "|-- " , node_UCT)
    ...
    def visualize_tree(self, root, depth=0, save_path=None):
        ...
        def add_nodes_edges(node, depth):
            ...
            for child in node.children.values():
                ...
                child_UCT = (
                        child.totalReward / (child.numVisits+1)
                        + self.explorationConstant_h
                        * math.sqrt(math.log(node.numVisits + 1) / (child.numVisits+1))
                    )
